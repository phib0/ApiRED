# Inspired by CodePulse (https://www.youtube.com/@CodePulse)
# All code from CodePulse has been written off of his tutorials and not copy-pasted from his GitHub
...
class Parser:
    ...
    def retreat(self, ):
        self.tok_idx -= 1
        if self.tok_idx < len(self.tokens) and self.tok_idx > -1:
            self.current_tok = self.tokens[self.tok_idx]
        return self.current_tok
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'trocken'):
            res.register_advancement()
            self.advance()
            ...
            if self.current_tok.type != TT_EQ and var == None:
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end, 
                    f"Expected '=' or identifier '{identifier}' hasn't been assigned"))
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if   node.op_tok.type == TT_PLUS:
            ...
            result, error = left.to_power_of(right)
...
global_symbol_table.set("Insimodus", Number(0))
